<script>
	...
	let zoom = 10;       // Zoom level
	let zoomTarget = 10;
	let zoomMinMax = [0.5, 10];
	let offsetX = w/2;    // X offset for panning
	let offsetXTarget = w/2;
	let offsetY = h/2;    // Y offset for panning
	let offsetYTarget = h/2;
	...
	const sketch = (p) => {
		...
		function checkDisplay(n) {
			...
			if (stage == "all_jobs" || stage == "all_jobs_middle") {
				zoomedGuidedTour = false;
				return true;
			}
			if (stage == "explore") {
		...
		function setStage() {
			...
			if (copy.story[currentStageNumber].stage == "one_similar_job") {
				centerAndZoomOnCoordinate(circles[maxIndicies[0]].center.x, circles[maxIndicies[0]].center.y, 10, 0.05);
			}
			if (copy.story[currentStageNumber].stage == "all_jobs_middle") {
				if (p.dist(circles[maxIndicies[0]].center.x, circles[maxIndicies[0]].center.y, w / 2, h / 2) < 200) {
					centerAndZoomOnCoordinate(w / 2, h / 2, 0.95, 0.05);
				} else {
					centerAndZoomOnCoordinate(circles[maxIndicies[0]].center.x, circles[maxIndicies[0]].center.y, 0.95, 0.05);
				}
			}
			...
			if (copy.story[currentStageNumber].stage == "all_jobs" || copy.story[currentStageNumber].stage == "all_jobs_middle") {
				centerAndZoomOnCoordinate(w/2, h/2, 0.95);
			}
		}
...
class Circle {
	constructor(obj, index) {
		...
		this.textDisplayed = false;
        		...
        		let sitterIndices = Array.from({ length: sitterNum }, (_, index) => index); // Create an array [0, 1, 2, ..., sitterNum - 1]
			p.shuffle(sitterIndices); // Shuffle the array using p5.js shuffle function

			let sitterIndexPool = [...sitterIndices]; // Clone the shuffled array to use as a pool

			for (let i = 0; i < this.peoplePositions.length; i++) {
			    // If the pool is empty, reshuffle and refill it
			    if (sitterIndexPool.length === 0) {
			        sitterIndexPool = p.shuffle([...sitterIndices]);
			    }

			    // Pop a random value from the pool
			    const randomIndex = sitterIndexPool.pop();
			    this.personTypeSitter.push(randomIndex);
			}
        	}
			...
			update() {
				...
				if (copy.story[currentStageNumber].stage == "all_jobs_middle" || copy.story[currentStageNumber].stage == "one_similar_job") {
					this.target.x = p.constrain(
						p.map(data[this.index].score_x, 0, 100, paddingX, w - paddingX),
						paddingX,
						w - paddingX
						);
				}
