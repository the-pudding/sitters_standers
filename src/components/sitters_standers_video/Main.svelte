<script>
	...
	import Canvas from "$components/sitters_standers_video/Canvas.svelte"; 
	import { fade } from 'svelte/transition';
	import Pulldown from "$components/sitters_standers_video/Pulldown.svelte"; 
	import Text from "$components/sitters_standers_video/Text.svelte"; 
	import Axis from "$components/sitters_standers_video/Axis.svelte"; 
	import Intro from "$components/sitters_standers_video/Intro.svelte";
	import copy from "$data/videocopy.json";

	export let data, headervars;
	...
	function updateData() {
		...
		data.forEach((item, i) => {
			...
			if (item.OCC_SHORT === "YOU" && selectedStandingPct == -1) {
				item.score = -1;
			} else if (copy.story[currentStageNumber].stage === "all_jobs_middle" || copy.story[currentStageNumber].stage === "one_similar_job") {
				if (item.yellow) {
					item.score = 50;
					...
					if (item.OCC_SHORT == "Laundry and Dry-Cleaning Workers") {
						item.score = 50;
						item.score_x = 50;
					} else {
						const radius = 32;  // Set the radius to 50 for values centered around 50 and within [0, 100]
						const centerX = 50; // Center X of the circle
						const centerY = 50; // Center Y of the circle

						// Generate random angle (0 to 2 * PI)
						const theta = Math.random() * 2 * Math.PI;

						// Generate random radius (square root for uniform distribution within circle)
						const r = Math.sqrt(Math.random()) * radius;

						// Calculate coordinates
						item.score = centerY + r * Math.sin(theta); // Y-coordinate (hovering around 50)
						item.score_x = centerX + r * Math.cos(theta); // X-coordinate (hovering around 50)
					}
				}
...
<div class="container">
	<span class="Stage">{timestamp}</span>
	...
	<div class="canvasContainer" transition:fade>
		...
		{/key} -->
		<Text bind:timestamp bind:searchValue {currentStageNumber} {copy} {data} {currentVar} on:updateCurrentVar={handleCurrentVarUpdate} on:updateQuestion={handleUpdateQuestion} bind:introActive  bind:questionNumber bind:reset/>
	</div>
	{/if}
</div>
